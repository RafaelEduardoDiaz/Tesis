...
<<fig.height=4, fig.cap="Datos homicidios: ACF y ACF de los PHMM con dos y tres estados.">>=
...
for(i in 1:h){
  ...
  plot(xf, fc, type = "h", main = paste("Forecast distribution for", año[59] + i), 
       xlim = c(5, max(xf+2)), ylim = c(0 ,0.1), 
...
<<fig.height=5, fig.cap="La distribución prónosticada para el año 2098">>=
...
PHMM_2states <- bayes.PHMM(y = Homicidios, m = 2, chains = 3, iter = 5000, thin = 3, 
                            control = list(adapt_delta = 0.99)))  # 57.07 seg
...
<<>>=
source("Bayes_fun.R")
bayes.viterbi(x = Homicidios, model = PHMM_2states, m = 2)
# bayes.lforward(x = Homicidios, model = PHMM_2states, m = 2)
bayes.state_prediction(h = 12, x = Homicidios, model = PHMM_2states, m = 2)
@
...
#mcmc_acf(posterior,pars=c("Gamma[1,1]","Gamma[1,2]","Gamma[2,1]","Gamma[2,2]","lambda[1]","lambda[2]"))
#mcmc_acf_bar(x=posterior,pars=c("Gamma[1,1]","Gamma[1,2]","Gamma[2,1]","Gamma[2,2]","lambda[1]","lambda[2]"))
@

<<>>=
traceplot(PHMM_2states)
@
...
<<results='asis'>>=
xtable(head(incendios), caption = "Grandes Incendios forestales, en Colombia desde enero del 2002 a diceiembre del 2016.")
@

<<>>=
table(incendios$GIF)
round(prop.table(table(incendios$GIF)),3)
@
