...
<<fig.width=6, fig.height=3.5, fig.cap="Función de autocorrelación muetral, para la seerie de homicidios.">>=
...
<<fig.height=4.7, fig.cap="Datos homicidios: ACF y ACF de los PHMM con dos y tres estados.">>=
...
<<fig.width=6, fig.height=4, fig.cap="Algoritmo Viterbi aplicado a un PHMM de dos estados.">>=
par(mfrow = c(1,1))
par(mar=c(2,2,1,.5)+.5, mgp=c(1.6,.6,0))
plot(Homicidios,xlab="Año",type='o', col=4, ylab = "Número")
rect(par("usr")[1], par("usr")[3],par("usr")[2],par("usr")[4],col=gray(.9,.9),border='white');grid(lty=1, col='white')
...
xtable(Estad_pred, caption = "Predicción de las probabilidades para un rezago h = 12.", digits = 4)
@


<<fig.height=4.5, fig.cap="Serie homicidios: distribución de pronóstico para el año 2019.">>=
#=== Use it for 1-step - ahead and plot the forecast distribution .
h <-1
xf <- 0:50
forecasts <- pois.HMM.forecast(xf, h, Homicidios, mod2)
par(mfrow = c(1 ,1), las=1, mar=c(2.1,2.5,1,.5)+.5, mgp=c(1.7,.3,0))
plot(xf ,forecasts , type ="h", xlim =c(0, max (xf)),ylim =c(0, 0.1), xlab = "conteos", ylab = "probabilidad", lwd=3)
rect(par("usr")[1], par("usr")[3],par("usr")[2],par("usr")[4],col=gray(.9,.9),border='white');grid(lty=1, col='white')
lines(xf ,forecasts , type ="h", lwd = 3)
@


<<fig.cap="Pronostico de la distribución para los años 2019 a 2022.">>=
...
for(i in 1:4){
  fc <- forecasts[, i]
  plot(xf, fc, type = "h", main = paste("Forecast distribution for", año[59] + i), 
       xlim = c(0, max(xf+2)), ylim = c(0 ,0.1), 
       xlab = "conteo", ylab = "probabilidad", lwd =3)
  rect(par("usr")[1], par("usr")[3],par("usr")[2],par("usr")[4],col=gray(.9,.9),border='white');grid(lty=1, col='white')
...
<<fig.height=5, fig.cap="La distribución prónostica">>=
...
plot(xf, fc, type ="h", main = paste (" Forecast distribution for ", año[59]+h), 
     xlim = c(0, max(xf)), 
     ylim = c(0 ,0.10), xlab ="conteo", ylab = "probabilidad", lwd =3)
rect(par("usr")[1], par("usr")[3],par("usr")[2],par("usr")[4],col=gray(.9,.9),border='white');grid(lty=1, col='white')
lines(xf, fc,type = "h", lwd = 3)
...
<<include=FALSE, cache=TRUE, eval=FALSE>>=
...
set.seed(1)
bridge_H0 <- bridge_sampler(samples = PHHMM_2states)
bridge_H1 <- bridge_sampler(samples = PHHMM_3states)
bridge_H2 <- bridge_sampler(samples = PHHMM_4states, method = "normal")
bridge_H3 <- bridge_sampler(samples = PHHMM_5states,repetitions = 1)

error_measures(bridge_H0)$percentage #"0.990%"
error_measures(bridge_H1)$percentage #"3.22%"
...
bf(bridge_H0, bridge_H1) # A favor de H0 3.10770
...
xtable(FactorBayes, caption = "Comparación resultados Factor de Bayes.")
@

<<include=FALSE, cache=TRUE>>=
system.time(
PHHMM_2states <- bayes.PHMM(y = Homicidios, m = 2, chains = 3, iter = 1000, 
                            control = list(adapt_delta = 0.99)))
@

Por lo tanto se elige un PHMM de orden 2 lo cual coincide con la teoría clásica. A continuación mostramos las estimaciones:

<<results='asis'>>=
resumenPHMM <- summary(PHHMM_2states)$summary
#colnames(resumenPHMM) <- c("Media","Err.Sta","Desv","2.5%","25%","50%","75%","97.5%","n_eff","Rhat")
#rownames(resumenPHMM) <- c("$\\A_{11}$","$\\A_{12}$","$\\A_{21}$","$\\A_{22}$","$\\lambda_1$","$\\lambda_2$","$lp__$")
print(xtable(resumenPHMM))#, sanitize.colnames.function = identity)
@

<<>>=
# Extract posterior draws for later use
posterior <- as.array(PHHMM_2states)
lp_cp <- log_posterior(PHHMM_2states)
np_cp <- nuts_params(PHHMM_2states)
...
<<>>=
traceplot(PHHMM_2states)
@

Para la estimación bayesiana primero vamos a elegir el número apropiado de estados utilizando el factor de Bayes

<<results='asis'>>=
resultado1 <- as.data.frame(summary(PHHMM_2states)$summary)
xtable(resultado1)
@

<<>>=
color_scheme_set("red")
mcmc_intervals(posterior, pars = c("Gamma[1,1]", "Gamma[1,2]", "Gamma[2,1]", "Gamma[2,2]"))
mcmc_intervals(posterior, pars = c("lambda[1]", "lambda[2]"))
@

<<>>=
library(coda)
PHMM_mcmc <- as.mcmc(as.matrix(PHHMM_2states))
