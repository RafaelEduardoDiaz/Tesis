...
print(xtable(Heidel1, caption = "Prueba de estacionariedad, usando el estadístico de Cramer-von-Mises para la convergencia de la cadena.", type = "latex"), sanitize.text.function = function(x){x})
print(xtable(Heidel2, caption = "Test de medio ancho para la media calculando el intervalo de confianza al 95 %.", type = "latex"), sanitize.text.function = function(x){x})
@
...
<<results='asis'>>=
tabla.incendios <- data.frame(cbind(table(incendios$GIF),round(prop.table(table(incendios$GIF)),3)))
...
acf(incendios$GIF, xlab="Rezago",main="", lag.max = 80)
rect(par("usr")[1], par("usr")[3],par("usr")[2],par("usr")[4],col=gray(.9,.9),border='white');grid(lty=1, col='white')
par(new = T)
acf(incendios$GIF, xlab="",main="", lag.max = 80)
@
