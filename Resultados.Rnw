...
<<fig.height=4.3, fig.cap="Datos homicidios: ACF y ACF de los PHMM con dos y tres estados.", fig.pos='h'>>=
...
points(x = homicidios[,1], y = ifelse(viterbi2 == 1, mod2$lambda[1], mod2$lambda[2]), pch = 16, col = "chartreuse2")
### Plot 2
...
<<results='asis'>>=
`año` <- homicidios[,1]
Estad_pred <- as.data.frame(pois.HMM.state_prediction(h = 12, x = Homicidios, mod = mod2))
colnames(Estad_pred) <- c("Estado 1","Estado 2")
rownames(Estad_pred) <- `año`[59]+1:12
Estad_pred$Estado <- apply(Estad_pred, 1, which.max)
xtable(Estad_pred, caption = "Predicción para las probabilidades de los estados hasta un rezago h = 12.", digits = 4)
@

<<fig.cap="Pronostico de la distribución para los años 2019 a 2030.">>=
...
<<fig.height=5, fig.cap="La distribución prónosticada para el año 2098", fig.pos='t'>>=
#=== Compute the marginal distribution ( called " dstat " below ) for mod2.
...
plot(xf,fc,type="h",main="",xlim=c(5,max(xf)),ylim=c(0,0.10),xlab="conteo",ylab="probabilidad",lwd =3)
rect(par("usr")[1], par("usr")[3],par("usr")[2],par("usr")[4],col=gray(.9,.9),border='white');grid(lty=1, col='white')
lines(xf, fc,type = "h", lwd = 3)
lines(xf, dstat, col = "chartreuse2", lwd = 3)
@
...
bridge_H2 <- bridge_sampler(samples = PHMM_4states, method = "normal")
bridge_H3 <- bridge_sampler(samples = PHMM_5states,repetitions = 1)

error_measures(bridge_H0)$percentage #"0.616%"
error_measures(bridge_H1)$percentage #"3.11%"
error_measures(bridge_H2)$percentage #"28.7%"
...
bf(bridge_H0, bridge_H2) # fuertemente a favor de H0 3047.46
bf(bridge_H0, bridge_H3) # fuertemente a favor de H0 390147608.97574

bf(bridge_H1, bridge_H2) # fuertemente a favor de H1 980.61632
...
<<results='asis'>>=
FactorBayes <- data.frame("mod 2 Estados" = c(NA, 3.36230, 3047.46, 390147608),
                          "mod 3 Estados" = c(NA, NA, 980.61, 125542040),
...
PHMM_2states <- bayes.PHMM(y = Homicidios, m = 2, chains = 3, iter = 5000, thin = 3, 
                            control = list(adapt_delta = 0.99)))  # 57.07 seg vs 13.80 seg
set.seed(3)
bridge_H0 <- bridge_sampler(samples = PHMM_2states)
...
print(xtable(resumenPHMM, caption = "Estimación bayesiana de los parámetros para un PHMM."), sanitize.rownames.function = function(x) {x})
@
...
points(x = homicidios[,1], y = ifelse(B_viterbi == 1, lambda_bayes[1], lambda_bayes[2]), pch = 16, col = "orange")
### Plot 2
...
rownames(Heidel1) <- c("$\\Gamma_{11}$","$\\Gamma_{21}$", "$\\Gamma_{12}$", "$\\Gamma_{22}$", "$\\lambda_1$", "$\\lambda_2$", "lp")

print(xtable(Heidel1, caption = "Prueba de estacionariedad, usando el estadístico de Cramer-von-Mises para la convergencia de la cadena.", type = "latex", digits = 3), sanitize.text.function = function(x){x})
@
...
rownames(Heidel2) <- c("$\\Gamma_{11}$","$\\Gamma_{21}$", "$\\Gamma_{12}$", "$\\Gamma_{22}$", "$\\lambda_1$", "$\\lambda_2$", "lp")

print(xtable(Heidel2, caption = "Test de medio ancho para la media calculando el intervalo de confianza al 0.95.", type = "latex", digits = 3), sanitize.text.function = function(x){x})
@
...
<<>>=
set.seed(201906)
muestras <- pois.HMM.generate_sample(ns = 10000, mod = mod2)
sum(pois.HMM.viterbi(x = muestras, mod = mod2) == bayes.viterbi(x = muestras, model = PHMM_2states, m = 2))
@
...
<<fig.width=6, fig.height=4, fig.cap="Serie de tiempo Grandes Incendios Forestales en Colombia desde el año 2002 hasta el año 2016.">>=
par(mar=c(2,2,1,.5)+.5, mgp=c(1.6,.6,0))
...
<<results='asis'>>=
xtable(head(incendios,12), caption = "Grandes Incendios forestales, en Colombia desde enero del 2002 a diceiembre del 2016.")
@

<<results='asis'>>=
tabla.incendios <- data.frame(cbind(table(GIF),round(prop.table(table(GIF)),3)))
colnames(tabla.incendios) <- c("Conteo","Proporción")
xtable(tabla.incendios, caption = "Tabla de frecuencia para la base GIF Colombia (2002-2016).")
@
...
ZIPHMM_2states <- hmmfit(y=incendios$GIF,
                         ...
                         emit_init=c(1, 5), zero_init = c(0.5,0),
...
ZIPHMM_3states <- hmmfit(y=incendios$GIF,
                         ...
                         tpm_init = matrix(c(0.8,0.1,0.1,
                                             0.1,0.8,0.1,
                                             0.1,0.1,0.8),3,3,byrow=TRUE),
                         emit_init=c(1, 2, 3), zero_init = c(0.5,0,0),
                         method="Nelder-Mead",hessian=TRUE,control=list(maxit=500,trace=1))

ZIPHMM_4states <- hmmfit(y=incendios$GIF,
                         M = 4, prior_init = c(0.4,0.2,0.1,0.3),
                         tpm_init = matrix(c(0.85,0.05,0.05,0.05,
                                             0.05,0.85,0.05,0.05,
                                             0.05,0.05,0.85,0.05,
                                             0.05,0.05,0.05,0.85),4,4,byrow=TRUE),
                         emit_init=c(1, 2, 3, 5), zero_init = c(0.5,0,0,0),
                         method="Nelder-Mead",hessian=TRUE,control=list(maxit=500,trace=1))

ZIPHMM_5states <- hmmfit(y=incendios$GIF,
                         M = 5, prior_init = c(0.4,0.2,0.1,0.2,0.1),
                         tpm_init = matrix(c(0.80,0.05,0.05,0.05,0.05,
                                             0.05,0.80,0.05,0.05,0.05,
                                             0.05,0.05,0.80,0.05,0.05,
                                             0.05,0.05,0.05,0.80,0.05,
                                             0.05,0.05,0.05,0.05,0.80),5,5,byrow=TRUE),
                         emit_init=c(1, 2, 3, 5, 7), zero_init = c(0.5,0,0,0,0),
                         method="Nelder-Mead",hessian=TRUE,control=list(maxit=500,trace=1))
@

<<results='asis'>>=
Tabla2 <- data.frame(
  Modelo = c("ZIPHMM - Estados 2", "ZIPHMM - Estados 3", "ZIPHMM - Estados 4", "ZIPHMM - Estados 5"),
  p = c(5, 10, 17, 26),
  logL = c(ZIPHMM_2states$nllk, ZIPHMM_3states$nllk, ZIPHMM_4states$nllk, ZIPHMM_5states$nllk),
  AIC = c(ZIPHMM_2states$aic, ZIPHMM_3states$aic, ZIPHMM_4states$aic, ZIPHMM_5states$aic),
  BIC = c(ZIPHMM_2states$bic, ZIPHMM_3states$bic, ZIPHMM_4states$bic, ZIPHMM_5states$bic))
...
plot(x = 2:5, y = Tabla2$AIC, type = "o", pch = 16, lty=2,xlim=c(1.5,6), ylim=c(470,650), xlab = "Número de estados", ylab = "")
rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4],col=gray(.9,.9),border='white');grid(lty=1,col='white')
lines(x = 2:5, y = Tabla2$AIC, type = "o", pch = 16, lty = 2)
lines(x = 2:5, y = Tabla2$BIC, type = "o", pch = 16, lty = 2)
text(x = c(5.3,5.3), y = c(543, 639), labels = c("AIC","BIC"))
@

<<>>=
ZIP.viterbi <- hmmviterbi(y = incendios$GIF, 
           ntimes = length(incendios$GIF), 
           M = 3, prior_init = ZIPHMM_3states$prior, 
           tpm_init = ZIPHMM_3states$tpm, 
           emit_init = ZIPHMM_3states$emit_parm, 
           zero_init = ZIPHMM_3states$zeroprop)
@
...
rect(par("usr")[1], par("usr")[3],par("usr")[2],par("usr")[4],col=gray(.9,.9),border='white');grid(lty=1, col='white')
lines(GIF, type='o', col=4)
abline(h = ZIPHMM_3states$emit_parm, col = "yellowgreen", lty = 2)
points(x = time(GIF), y = ifelse(ZIP.viterbi == 1, ZIPHMM_3states$emit_parm[1],ifelse(ZIP.viterbi == 2,ZIPHMM_3states$emit_parm[2],ZIPHMM_3states$emit_parm[3])), pch = 16, col = "orange")
@
