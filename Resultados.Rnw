...
<<results='asis'>>=
source("Bayes_fun.R")
B_vitervi <- bayes.viterbi(x = Homicidios, model = PHMM_2states, m = 2)
# bayes.lforward(x = Homicidios, model = PHMM_2states, m = 2)
df3 <- rbind(B_vitervi[1:20], B_vitervi[21:40], B_vitervi[41:60])
...
xtable(B_state_pred, caption = "Predicción bayesiana de las probabilidades para un rezago h = 12.", digits = 4)
@
